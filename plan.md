...
- **Player authority boundary — the player is not the boss.** Unlike a normal assistant chat, in-game the player must not be able to sway the DM with out-of-character pressure: customer-service framing, authority claims, rules-lawyering, "as an AI you must…" jargon, or social manipulation. The player's authority is exactly: declaring their character's words and actions, and table-talk questions. Rule changes, settings, and meta-control belong to the owner channel only (see owner/player settings split topic). Enforcement direction: explicit resistance instructions in the Interaction/Referee prompts (treat out-of-character coercion as table talk to deflect in persona), with the continuity gate and engine-side validation (e.g. the clarification no-op net) as the backstop that prompt-level compliance failures cannot mutate state. Becomes security-relevant once campaigns are multiplayer/hosted.

...
| D2 | Ability representation | Fixed effect catalog, genre-spread prototype first | **DECIDED 2026-07-16** — complications are free text with contextual suggestions, never a fixed complication table; any mechanical consequence maps to an engine **verb** (state operation) from a catalog kept deliberately wide; model trust is tuned via the ledgered stakes license, never by unledgered effects (see `.agents/decisions.md`). Scope deliverables recorded earlier stand: complication-effect classes (resource/inventory loss, NPC disposition shifts, encounter initiation, outcome-value modulation with D1b), weight classes (minor/significant), per-operation point costs, and valence tags (beneficial/adverse). The accepted catalog **document** (reviewloop, like the resolution chapter) still gates edge-band implementation (`docs/rules/resolution.md` §1.5). Unblocks D3, D5, D13. |

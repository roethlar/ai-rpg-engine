...
| ds-1 | MEDIUM | Pre-existing: choice buttons allow overlapping submits — duplicated transcript entries and a mid-turn UI lie | `[~]` fixed, pending reviewer verdict (three quarters had already landed unguarded in `f042082`; now pinned) | `fix/ds-1-submit-reentrancy` @ `2f893e1` |
| fk-1 | MEDIUM | Pre-existing: a fork resolving after the user left (keyboard path) silently seizes the table | `[x]` verified; awaiting owner-gated merge (codex/gpt-5.6-sol/xhigh — guard audited, not reviewer-executed; see jt-1) | `fix/fk-1-fork-epoch` @ `60df275` |

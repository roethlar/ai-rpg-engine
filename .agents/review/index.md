...
| css-2 | MEDIUM | The css-1 guard scans only `styles.css`, but `index.html` (inline styles), `app.js` and `map-render.js` also author theme-var CSS — the same defect can be reintroduced there with the suite green | `[~]` REOPENED at r2 — 5 executed bypasses (cross-file alias, partial entity decode, `<style>` RAWTEXT, non-string-aware CSS comments, map-render.js). **Scoping question routed to owner** | `fix/css-2-scanner-scope` @ `ff77b95` |

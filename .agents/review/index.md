...
| jt-1 | HIGH | Pre-existing: Journal tab renders a stale campaign's history over the current one (empirically confirmed); Fork buttons then fork the wrong campaign | `[~]` fixed, pending reviewer verdict | `fix/jt-1-journal-epoch` @ `09768e1` |

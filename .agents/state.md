...
- **REUSABLE PRIVATE PLAYER KEYS ARE SELECTED; LOST-KEY RECOVERY AUTHORITY IS NEXT.** The 2026-09-05 decision in `.agents/decisions.md` chooses private keys over account sign-in for cross-campaign library ownership. Section 3.3 of `.agents/review/campaign-character-version-plan.md` drafts stable player ownership, enrollment/claims, explicit host/player/seat privacy, rotation and recovery. The remaining question is administrator-assisted recovery versus a separate player-held recovery secret. No key is implemented or generated by this planning decision.
...
**NEXT:** Obtain the lost-key recovery ruling in section 10, item 1 of the version-plan draft.
Recommend administrator-assisted recovery using explicit admin authority and a one-use grant;
the alternative is a player-held recovery secret with no product admin reset. Record the answer,
finish the recovery contract, then present catalog compatibility as the next individual decision.
Private key versus account sign-in is already settled. No whole-plan or code approval is implied.
...
- Phase PT's historical one-record plan is superseded. The version architecture is now drafted,
  but key-recovery authority, compatibility/legacy handling, Creator description, rules/catalog content
...
- `.agents/review/campaign-character-version-plan.md` — current unapproved version architecture, settled player-key direction, next recovery decision, proposed delivery and verification gates.

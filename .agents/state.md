...
**NEXT:** Seek one owner ruling on the paired-playtest draft. Approval authorizes only IBP-1 (exact pilot fixture/schema/validator) and IBP-2 (offline player runner/instrumentation), one committed slice at a time, then stops for the owner's human playtest. It does not authorize a tier result, later pair, product integration, reviewer, or push. Separately, no further Phase PT implementation may proceed until an owner-approved revision incorporates campaign sets, safe upgrades, class migrations, and player-owned character versions. The prior Fable items `rsv-1` and `rsv-4` remain downstream.
...
- `.agents/review/interaction-burden-playtest-plan.md` — concrete offline Armsmaster/Adept pilot and harness contract; draft only, no implementation or playtest result authorized.

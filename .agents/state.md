...
- Heroic/map layout mocks at `docs/mockups/heroic-layouts.html` (also published as artifact). Owner verdicts 2026-07-03: static three — B best, A wastes a rail, C too squeezed; Layout D (dynamic spotlight: one info rail, click-to-focus any surface) judged GOOD, with one refinement applied: map and grounding text always coexist on positional turns (complements, never either/or — map on top, text as caption). Direction is effectively settled on D; promoting the maps/heroic phase still needs the remaining design pieces (heroic identity anchors, image-gen provider seam, structured location state) scoped into a concrete phase entry.

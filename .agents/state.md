...
- **THE PRODUCTION ABILITY-KEYWORD PLAN IS OWNER-APPROVED; AKP-1 IS COMPLETE.** `.agents/review/ability-keyword-production-plan.md` authorizes AKP-1 through AKP-4 in order, one verified commit per slice. AKP-1 supplies the shared deterministic matcher/insertion helpers, canonical server-owned trigger projection and revision digest, inert live party projection, and seat-safe trigger whitelist. Existing free-text abilities remain non-invocable because they have no catalog invocation metadata, registered family, or complete campaign binding. AKP-2 is next: authenticated server recomputation, structured Council declarations, invocation persistence/export, and spoof/stale guards. AKP-3 may then build the textarea UI. AKP-4 remains blocked until the real versioned class/catalog creator supplies complete stable abilities and bindings; prototype fixtures cannot fill that gap. No real campaigns need generated-rules-card migration, code never deletes disposable test campaigns automatically, and no generated-card converter or dual trigger path is built.
...
**NEXT:** Implement AKP-2 from the approved production ability-keyword plan: revision-only turn
request, authenticated server recomputation, structured Council declarations, bounded invocation
persistence/export/fork handling, and forged/stale authority guards.

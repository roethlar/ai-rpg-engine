...
**NEXT:** On explicit authorization, draft a concrete paired-scenario playtest plan and harness contract using the same character and encounter with only the candidate mechanic changed. The draft must capture the settled observations and propose no paper interaction gate or tier membership. No harness implementation, package regeneration, or reviewer invocation is authorized. Separately, no further Phase PT implementation may proceed until an owner-approved revision incorporates campaign sets, safe upgrades, class migrations, and player-owned character versions. The prior Fable items `rsv-1` and `rsv-4` remain downstream.

...
- **THE SECOND COLLAPSE PROTOTYPE SUPPORTS ADEPT AS A SEPARATE UNIVERSAL CANDIDATE; THE OWNER HAS NOT RULED.** `.agents/review/archetype-collapse-prototypes.md` gives Armsmaster and Adept the same d100 bonus, durability, weapons, action budget, and reaction cap. Armsmaster freely selects a complete Form each turn; Adept's character-owned opening/flow/finishing state restricts later legal techniques and resets after a non-technique action. Stable, moving-rescue, miss/disruption, weapon, and cave-era regressions show that flattening techniques into Forms either exposes finishers immediately or recreates the sequence engine inside Armsmaster. The remaining risks are a solved rotation, package-specific balance, UI clarity, and cheap multiclass stacking.
...
**NEXT:** Present the Armsmaster-versus-Adept recommendation for an owner ruling. On an explicit go, run the final Leadership-versus-Catalyst collapse prototype. Intruder is no longer a roster candidate. Do not regenerate the three frozen rules packages, implement runtime changes, or invoke another reviewer. The prior Fable review items `rsv-1` and `rsv-4` remain open but are downstream of the roster problem.
...
- `.agents/review/archetype-collapse-prototypes.md` — matched-card collapse record: owner removed Intruder after prototype 1; prototype 2 supports separate Armsmaster and Adept candidates pending owner ruling.

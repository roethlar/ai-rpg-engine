...
- **THE PRODUCTION ABILITY-KEYWORD PLAN IS OWNER-APPROVED; AKP-1 AND AKP-2 ARE COMPLETE.** `.agents/review/ability-keyword-production-plan.md` authorizes AKP-1 through AKP-4 in order, one verified commit per slice. AKP-1 supplies the shared deterministic matcher/insertion helpers, canonical server-owned trigger projection and revision digest, inert live party projection, and seat-safe trigger whitelist. AKP-2 adds the exact-prose/revision-only request boundary, authenticated server recomputation, one immutable declaration record for every Council role, stale-before-model handling, and a separate bounded invocation record preserved through recent history, bundle v3 import/export, and forks while remaining absent from seat journals. Unsupported client authority fields are rejected. Existing free-text abilities remain non-invocable because they have no catalog invocation metadata, registered family, or complete campaign binding. AKP-3 is next: the accepted native-textarea mirror, exact highlighting, insertion, spelling recovery, revision echo, and stale/retry/session behavior. AKP-4 remains blocked until the real versioned class/catalog creator supplies complete stable abilities and bindings; prototype fixtures cannot fill that gap. No real campaigns need generated-rules-card migration, code never deletes disposable test campaigns automatically, and no generated-card converter or dual trigger path is built.
...
**NEXT:** Implement AKP-3 from the approved production ability-keyword plan: the production native
textarea/mirror, exact owned-term highlighting, ability-button insertion, suggestion-only spelling
recovery, opaque revision echo, and safe stale/network/session recovery.
Final catalog cutover still waits for the real versioned class/catalog producer rather than
provisional prototype abilities. Do not resume the rejected IBP-2 runner. Separately, no further Phase PT
...
- AKP-1: `node test.js`, `node --check public/ability-keywords.js`, `node --check ability-trigger-state.js`,
  and `git diff --check` are green on 2026-08-03. Guard proof temporarily disabled normalized
  trigger-collision rejection; the dedicated collision assertion failed, then the restored code
  returned the full suite green. Coverage includes exact/alias matching, Unicode UTF-16 ranges,
  word boundaries, longest overlap, typo suggestion without activation, passive/legacy exclusion,
  binding/family validation, revision invalidation, live inert projection, and seat isolation.
- AKP-2: `node test.js`, syntax checks for every changed JavaScript file, and `git diff --check`
  are green on 2026-08-03. Guard proof temporarily accepted unsupported request fields; the forged
  cross-character `abilityIds` assertion failed, then the restored request allowlist returned the
  full suite green. Coverage includes exact untrimmed prose, shared scan parity, host/seat speaker
  selection, stale 409 before model/write, immutable Council context and model-field overwrite,
  bounded range validation, corrupt-history fail-closed behavior, opening empty records, bundle-v3
  import remapping, fork identity, older bundle normalization, and seat journal isolation.

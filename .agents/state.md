...
- **THE PRODUCTION ABILITY-KEYWORD PLAN IS DRAFTED, NOT APPROVED.** `.agents/review/ability-keyword-production-plan.md` specifies stable catalog-backed character abilities, campaign trigger bindings, one shared browser/server matcher, authenticated server recomputation, structured Council declarations, invocation persistence/export, seat isolation, and the accepted textarea UI. The owner ruled that no real campaigns need generated-rules-card migration; disposable local test campaigns may be deliberately wiped, but code never deletes them automatically and no generated-card converter or dual trigger path is built. Final cutover remains blocked until the real versioned class/catalog creator supplies complete stable abilities and bindings; prototype fixtures cannot fill that gap.
...
**NEXT:** Present the drafted production ability-keyword plan for owner approval; no implementation
is queued. Even after approval, final catalog cutover waits for the real versioned class/catalog
producer rather than provisional prototype abilities. Do not resume the rejected IBP-2 runner or
infer product authorization from prototype acceptance. Separately, no further Phase PT

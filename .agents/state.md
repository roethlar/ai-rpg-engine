...
- **QUEUED, PLANNED, NOT STARTED:**
  ...
  - **v-1 ACTIVE** on `fix/v-1-grok-provider-config`: provider registry/request contract,
    provider-isolated key/config compatibility, admin storage, and guards are implemented and green;
    independent review is next. Status and verdict trail: `.agents/review/index.md` +
    `findings/v-1.md`.

...
**NEXT:** Await an explicit go to run the audit's three collapse prototypes: Armsmaster/Adept, Opportunist/Intruder, and Leadership/Catalyst. Use their concrete cards and scenario results to settle the roster before regenerating the three frozen rules packages; do not implement runtime changes or invoke another reviewer. The prior Fable review items `rsv-1` and `rsv-4` remain open but are downstream of the roster problem.

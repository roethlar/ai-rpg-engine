...
- Unscheduled (needs phase promotion): enforce server-owned AI config per the 2026-06-11 decision in `.agents/decisions.md` — client `apiConfig` currently overrides server env keys/provider/model. Also unscheduled: remaining 2026-06-11 code-review findings (denied-action dice damage in rules_mode, removed SVG quote-escape prompt rule, SVG omitted from turn prompt output list, triplicated clarification zeroing — the latter two may be mooted/absorbed by the visualizer-map direction and Council refactor respectively). Fixed 2026-06-11 night: Grok endpoint pin, per-role cross-provider key inheritance, opening-turn clarification wipe guard. New finding while fixing: callClaude honors CUSTOM_ENDPOINT_URL/baseUrl the same way callGrok did (same key-leak pattern, pre-existing) — unfixed, queued.

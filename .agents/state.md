...
- **IBP-1 REMAINS EVIDENCE; THE OWNER REJECTED IBP-2'S PLAYER INTERACTION.** The committed immutable Armsmaster/Adept fixture remains valid evidence, but manual evaluation found the uncommitted prose-plus-separate-action-selection runner unrepresentative and excruciating. Players must use one fiction-first text submission rather than operate Forms, sequence state, target menus, or designer vocabulary. The rejected runner remains uncommitted and untouched pending explicit disposal authority. `.agents/review/ability-keyword-composer-plan.md` is a draft for a non-shipping UI proof: exact owned ability words highlight inline, ability-card clicks insert canonical words at the caret, and fuzzy spelling recovery never silently activates. No replacement implementation or product integration is approved.
...
**NEXT:** Present the fiction-first ability-keyword composer plan for an owner ruling. If approved,
implement its two non-shipping prototype slices and stop for owner evaluation; do not resume the
rejected IBP-2 runner or integrate the feature into the product. Separately, no further Phase PT
...
- `.agents/review/ability-keyword-composer-plan.md` — draft non-shipping proof for inline exact-word recognition, ability-card insertion, and non-activating spelling recovery; no code approved.

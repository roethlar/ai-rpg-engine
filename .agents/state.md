...
- **THE FIRST COLLAPSE PROTOTYPE SUPPORTS INTRUDER ONLY AS A CONDITIONAL CLASS.** `.agents/review/archetype-collapse-prototypes.md` gives matched Opportunist and Intruder cards the same d100 SkillBonus, tools, and Main-action budget, then runs them through a cyberpunk data vault and a Neanderthal protected route. One local intrusion collapses to an Opening or common skill. A qualifying multi-node system does not: banked system-scoped Access, exact permissions, and Alert-driven route changes create repeated choices that one target-bound Opening cannot reproduce without rebuilding the Intruder subsystem. The result is mechanical-identity evidence, not roster approval or numerical balance; Armsmaster/Adept and Leadership/Catalyst remain untested.
...
**NEXT:** Present the Intruder-versus-Opportunist verdict for an owner ruling. On an explicit go, run Armsmaster versus Adept next with matched cards and scenario results; Leadership versus Catalyst remains after it. Do not regenerate the three frozen rules packages, implement runtime changes, or invoke another reviewer. The prior Fable review items `rsv-1` and `rsv-4` remain open but are downstream of the roster problem.
...
- `.agents/review/archetype-collapse-prototypes.md` — first matched-card collapse result; supports Intruder only as a conditional class, pending owner ruling.

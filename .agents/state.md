...
- **THE PRODUCTION ABILITY-KEYWORD PLAN IS OWNER-APPROVED.** `.agents/review/ability-keyword-production-plan.md` authorizes AKP-1 through AKP-4 in order, one verified commit per slice: stable catalog-backed character abilities, campaign trigger bindings, one shared browser/server matcher, authenticated server recomputation, structured Council declarations, invocation persistence/export, seat isolation, and the accepted textarea UI. AKP-1 through AKP-3 may build inert generic infrastructure. AKP-4 remains blocked until the real versioned class/catalog creator supplies complete stable abilities and bindings; prototype fixtures cannot fill that gap. No real campaigns need generated-rules-card migration, code never deletes disposable test campaigns automatically, and no generated-card converter or dual trigger path is built.
...
**NEXT:** Implement AKP-1 from the approved production ability-keyword plan: shared deterministic
matcher, canonical server-owned trigger projection, revision digest, and seat-safe state projection.

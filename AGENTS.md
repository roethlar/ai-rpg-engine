...
- Core principle (evidence: plan.md): Every change must improve *fun* and *feel* like a real GM. Avoid feature creep. Prioritize quality of interaction over new mechanics.
- Work is organized into explicit phases. Items in "Future Topics" (plan.md) are for discussion only. Per project rule (plan.md): nothing may be implemented until it is promoted into a concrete phase with planned entries, success metrics, and files to change.
- Phase review gate (plan.md): After completing a phase's implementation work, run a full play session (ideally Council DM + strong or local model), gather feedback, and confirm the changes demonstrably improve the playing experience before treating the phase as complete or advancing. No code is merged/landed until it passes this gate.
- The Council DM pipeline is the supported path for player turns (Interaction, Continuity, Referee, Continuity final check, Narration). Single-model paths are deprecated per current plan.
...
Current automated verification command for this repo (evidence: package.json:10 "test": "node test.js", confirmed in repo-discovery-manifest.json verificationCandidates and direct read of package.json and test.js; no CI workflows present per scan of root and absence in review packet):

- `node test.js` (or `npm run test`)

This runs unit tests covering parseJsonSafe, level-up math, production SSRF endpoint protection, JSON schema validation + clamping (including Phase 0 clarification no-op safety net), dice check math, and task queue concurrency serialization. All must pass for code changes.

Manual/playtest requirements (from project process in plan.md): for Phase work and user-visible behavior changes, run full play sessions and confirm improvement per the review gate.
